...
samp1 %>%
  summarise(mus1 = mean(area), pop_meds1 = median(area),
          sigmas1 = sd(area),pop_iqrs1 = IQR(area),
          pop_mins1 = min(area), pop_maxs1 = max(area),
          pop_q1s1 = quantile(area, 0.25),
          pop_q3s1 = quantile(area, 0.75))
...
sample_means_small %>%
  summarise(length(sample_means_small$x_bar))
```
...
for (i in 1:5000) {
  sample_means50[i] <- mean(sample(ames$price, 50))
}
hist(sample_means50)
mean(ames$price)
sd(ames$price)
...
for (i in 1:5000) {
  sample_means150[i] <- mean(sample(ames$price, 150))
}
...
sample_means15 <- rep(NA, 2000)
for (i in 1:2000){
  sample_means15[i]<-mean(sample(ames$price,15))
}
mean(sample_means15)
```
...
for (i in 1:2000){
  sample_means150[i]<-mean(sample(ames$price,15))
}
mean(sample_means150)
```
